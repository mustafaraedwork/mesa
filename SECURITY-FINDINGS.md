...
| H-3 | Lazy-revert كتابتان غير ذرّيتين عبر جدولين (نافذة تسابق تنتج حالة وسطية — خلل UX) | HIGH | `lib/menu.ts:134-148` · `app/api/admin/state/route.ts:47-64` | ⏳ كود جاهز · `0008` يدوي | `applyLazyRevert()` يحاول `rpc('revert_closing_mode')` ويسقط بأمان إلى البيانين؛ الذرّية الكاملة تُفعَّل بعد تطبيق `supabase/migrations/0008_atomicity_rpcs.sql` يدويًا على Supabase. |
...
| M-3 | سياسة RLS للمالك تكشف عمود `token` الخام في `tenant_sessions` (database-reviewer صنّفه Critical؛ خُفِّض لأنه owner-only موثوق) | MEDIUM | `supabase/migrations/0001_init.sql` (سياسة Owner full access) | ⏳ `0009` يدوي | `REVOKE SELECT (token) ON tenant_sessions FROM anon, authenticated` في `0009_db_hardening.sql` (service-role يتجاوزه ويبقى يقرأه للتحقّق). يحتاج تطبيقًا يدويًا على Supabase. |
