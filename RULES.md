...
- **استثناء لوحة الأونر (2026-06-06):** يُسمح بـ**تسجيل** الدفعات يدوياً (دفتر offline) + تتبّع **تجديد سنوي اختياري** + تحليلات إيراد المنصّة + `branch_count` تسعيري. يبقى ممنوعاً: **معالجة** الدفع أونلاين (بوابة)، multi-branch فعلي، وتقارير مبيعات المطاعم. التفصيل في `prd.md` §8 و`OWNER-PANEL-PLAN.md`. (ملاحظة: §2 «لا audit logs» يبقى سارياً — لا سجل تدقيق.)
