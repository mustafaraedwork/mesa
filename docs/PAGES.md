...
| **التحديث الحي** | استطلاع كل **30 ثانية** على `/api/menu/{slug}` (`menu-view.tsx:31 POLL_MS`, `:107`)، يتوقّف عند `document.hidden`، يستأنف على `visibilitychange`. الاستجابة مخزّنة على CDN Vercel لـ**10 ثوانٍ** (+15 stale-while-revalidate) عبر `Vercel-CDN-Cache-Control` والمتصفّح لا يخزّنها → أقصى تأخير لتغيير يصل الزبون **40 ثانية** |
