# BIZIII Menu – PRD (Product Requirements Document)

> ⚠️ **هذه الوثيقة متأخّرة عن الكود ولا تُعتمد كمصدر حقيقة.**
> كُتبت قبل التنفيذ وبقيت على نسخة 0.5 بينما تغيّر الكود بعدها (٧ جداول لا ٥، أوضاع
> `normal/closing/off` بعد إسقاط rush/profit، تبويب تحليلات، لوحة فوترة، Next 16).
> للوضع الفعلي راجع **`docs/PAGES.md`** (جرد الصفحات مقروء من الكود) و**`docs/COMPANY-CONTEXT.md`**
> (سياق الشركة وقواعد العزل). استخدم هذه الوثيقة للنيّة الأصلية والقرارات التاريخية فقط.
...
**BIZIII Menu** منصة SaaS **باشتراك سنوي** للمطاعم والكافيهات، توفر:

> ### 🔴 تصحيح 2026-08-25 — نموذج البيع
> **الوصف السابق «ذات بيعة واحدة (One-time purchase)» ملغى بقرار المالك.**
> البيع **اشتراك سنوي**، بثلاث باقات بالدينار العراقي:
>
> | الباقة | المحتوى | السعر السنوي |
> |---|---|---|
> | `menu` | Menu وحده | ١٩٩٬٠٠٠ |
> | `growth` | Menu + Feedback (المستهدفة) | ٤٩٩٬٠٠٠ |
> | `growth_pro` | — | ٦٩٩٬٠٠٠ |
>
> **سلوك الانتهاء المعتمَد** (منفَّذ في الهجرة `0016` و`lib/subscription.ts`):
> - **مهلة ٣ أيام** بعد `period_end`: كل شيء يعمل — المنيو ولوحة التحكّم — مع
>   تحذير متدرّج في لوحة المستأجر يعرض الأيام المتبقّية.
> - **بعد اليوم الثالث: تعليق كامل** — لوحة المستأجر **ومنيو الزبون العام** معاً.
>   يرى الزبون شاشة محايدة («المنيو الرقمي متوقّف مؤقتاً») لا شاشة «مغلق»، ولا
>   تُذكر الفوترة أمامه إطلاقاً.
> - **تسجيل دفعة تمدّ `period_end` يُعيد كل شيء فوراً** بلا أي تدخّل إضافي.
> - **مطعم بلا أي دفعة** (`subscription_ends_at IS NULL`) يُعامَل **نشطاً**، لا
>   معلَّقاً — الحساب يُنشأ قبل تسجيل دفعة التأسيس.
> - `is_active` (تعطيل يدوي من المالك) يبقى **مستقلاً تماماً** عن التعليق
>   التلقائي بالانتهاء؛ سببان مختلفان ولا يُدمجان.
>
> يبقى ممنوعاً: **معالجة** الدفع أونلاين، multi-branch فعلي، تقارير مبيعات
> المطاعم (`RULES.md` §3).

- منيو رقمي ذكي يعرض على الزبون عبر QR Code
- لوحة تحكم PWA لصاحب المطعم تعمل من الهاتف
- وضعان ذكيان للعرض (عادي / إغلاق) – الميزة المميزة (السرعة والربح أُلغيا 2026-05-26)
...
| نسبة الربح % | ✅ | للحسابات الداخلية (وضع رفع الأرباح أُلغي 2026-05-26) |
| وقت التحضير | ✅ | بالدقائق – يظهر للزبون ⏱ (وضع السرعة أُلغي 2026-05-26) |
...
**القاعدة:** وضع واحد فقط مفعّل في كل لحظة. **(تحديث 2026-05-26: تبقّى وضعان فقط — عادي + إغلاق.)**

> **🗑️ مُلغى 2026-05-26 (قرار المالك):** وضعا **السرعة (Rush)** و**رفع الأرباح (Profit)** أُزيلا. السبب: بعد إعادة تصميم المنيو (شرائح فلتر تعرض سكشناً واحداً) صارا مجرّد إعادة ترتيب داخل السكشن بلا أي إشارة بصرية للزبون وبلا رجوع تلقائي — قيمتهما المضافة فوق الترتيب اليدوي في الوضع العادي تكاد تكون معدومة. **حقلا `profit_percentage` و`prep_time_minutes` يبقيان** كحقلَي منتج (للحسابات الداخلية والعرض)؛ فقط الوضعان أُزيلا. قيد الـDB ضُيِّق في migration `0004`.

##### الوضع 1: عادي (Normal)
- الترتيب اليدوي للسكاشن والمنتجات
- لا توجد تعديلات على العرض
- **اختيارات الشيف (2026-05-26):** يمكن للعميل تحديد أصناف مميّزة (`is_chef_pick`) من زرّ "تعديل اختيارات الشيف" في تبويب الأوضاع — تظهر في قسم "اختيارات الشيف" بأعلى المنيو **بلا خصم**. اختيار فارغ = لا قسم.

##### ~~الوضع 2: السرعة (Rush Hour)~~ — مُلغى
##### ~~الوضع 3: رفع الأرباح (Profit Mode)~~ — مُلغى

##### الوضع 2: الإغلاق (Closing Mode)
...
##### الوضع 3: إيقاف (Off) — 2026-05-26
- زرّ "إيقاف كل الأوضاع" بجانب بطاقتي الأوضاع
- الزبون يرى **منيو عادي فقط**: لا عروض إغلاق، لا قسم/عنوان "اختيارات الشيف"، ولا الترحيب التحريري — السكشنات والأصناف فقط
- مختلف عن **تعطيل الحساب** (`is_active = false`) الذي يُظهر شاشة "المطعم مغلق"؛ هنا المنيو يظهر لكن بلا أي زينة أوضاع
- اختيارات الشيف المحفوظة (`is_chef_pick`) تبقى وتعود عند تفعيل الوضع العادي
...
CREATE TABLE restaurants (
  ...
  -- migration 0007 (2026-05-26): NULL → app defaults (header→background, card→#fff, text→#1a1a1a)
  header_color TEXT,
  card_color TEXT,
  text_color TEXT,
  ...
  -- Mode state (rush/profit retired in migration 0004; 'off' added in 0006 — both 2026-05-26)
  active_mode TEXT DEFAULT 'normal' CHECK (active_mode IN ('normal','closing','off')),
...
CREATE TABLE products (
  ...
  -- Chef's Picks flag for Normal mode (migration 0005, 2026-05-26)
...
**تحديث (إصلاحات القدرة P0):** استجابة `/api/menu/{slug}` تُخزَّن على CDN لمدة 10 ثوانٍ (المتصفّح لا يخزّنها) حتى تتجمّع استطلاعات كل زبائن المطعم في تصيير واحد. النتيجة: أي تغيير (سعر، وضع، صنف) يصل الزبون المتصل خلال **40 ثانية كحدّ أقصى** (30 استطلاع + 10 كاش).
...
| T7 | الأوضاع | عرض الوضعين (عادي/إغلاق) + active state |
...
- [x] منطق الأوضاع (٤ أوضاع أصلاً → قُلِّصت لوضعين: عادي/إغلاق، 2026-05-26)
...
### ضمن النطاق — توضيح لوحة الأونر (2026-06-06)

لوحة الأونر (`/owner`) تشمل إدارة فوترة المنصّة الداخلية. هذا **ليس** نقضاً لما سبق:

- ✅ **تسجيل الدفعات يدوياً** (دفتر offline): الأونر يسجّل ما استلمه فعلياً من المطاعم. هذا **تسجيل**، لا **معالجة** — لا بوابة دفع.
- ✅ **تتبّع تجديد سنوي اختياري**: حقل فترة على الدفعة يشتقّ «التجديد القادم/المتأخر» وقت القراءة. لا محرّك اشتراكات، لا cron.
- ✅ **تحليلات إيراد المنصّة**: دخل الأونر (Mustafa) من بيع التراخيص — مؤشّرات تجميعية لكل عملة.
- ✅ **عدّاد فروع للتسعير** (`branch_count`): رقم تسعيري فقط، **بلا** أي وظائف multi-branch.

يبقى **خارج النطاق صراحةً**: ❌ معالجة الدفع أونلاين (بوابة)، ❌ multi-branch فعلي (قوائم/حسابات منفصلة لكل فرع)، ❌ **تقارير مبيعات المطاعم** (تحليلات الأونر تخصّ إيراد المنصّة، لا مبيعات طعام أي مطعم).
