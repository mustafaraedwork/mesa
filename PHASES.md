...
**الهدف:** الأوضاع تشتغل وتنعكس على واجهة الزبون. **(2026-05-26: قُلِّصت من ٤ إلى وضعين — عادي/إغلاق؛ السرعة والربح أُلغيا — migration `0004`.)**

- [x] صفحة `/admin/dashboard/modes`:
  - [x] عرض البطاقات + المفعّل بحالة بصرية مميزة (`ring-primary` + شارة "نشط") — بطاقتان بعد الإلغاء
...
- [x] منطق إعادة الترتيب في API layer (`/api/menu/[slug]`):
  ...
  - [x] ~~Rush: `prep_time_minutes ASC`~~ — مُلغى 2026-05-26
  - [x] ~~Profit: `profit_percentage DESC`~~ — مُلغى 2026-05-26
- [x] **اختيارات الشيف (الوضع العادي)** — 2026-05-26: عمود `is_chef_pick` (migration `0005`) + نافذة multi-select بزرّ "تعديل اختيارات الشيف" على بطاقة العادي (`setChefPicks`) → فئة وهمية "اختيارات الشيف" بأعلى المنيو بلا خصم
- [x] **وضع الإيقاف `off`** — 2026-05-26: قيمة ثالثة (migration `0006`) + زرّ "إيقاف كل الأوضاع" → منيو عادي بلا أي عروض/قسم اختيارات شيف/ترحيب (مختلف عن تعطيل الحساب)
