...
- **المرحلة الحالية:** 🟢 إعادة تصميم صفحة المنيو الرئيسية للزبون — مكتملة
- **آخر إنجاز:** menu-view الجديد: هيدر مدمج + عنوان مجلّة + شريط رقاقات فلتر بمستويين + قسم "اختيارات الشيف" (إعادة تسمية "عروض اليوم") + شبكة منتجات بعمودين + شريط سلة عائم
- **التالي:** مهام Mustafa — نشر Coolify، التحقّق اليدوي. (`smoke-runtime-confirms.mjs` اختبار قديم يرصد `window.confirm` المهجور — يُستحسن حذفه؛ البديل `smoke-no-native-confirms.mjs`)
- **عقبات/قرارات معلّقة:**
  - اسم الدومين (PRD §٩)
  ...
  - **🟡 مؤجَّل — `is_chef_pick` للوضع العادي:** عمود boolean جديد على `products` + checkbox في نموذج المنتج. عند تفعيله، قسم "اختيارات الشيف" يظهر في الوضع العادي أيضاً بالمنتجات المُعلَّمة. الحالي: القسم يظهر فقط في وضع الإغلاق.
  - **🟡 مؤجَّل — شريط التاريخ/الوقت في الهيدر:** "TUESDAY · HH:MM" بحروف لاتينية صغيرة كما في نظام التصميم. حُذف من تنفيذ هذه المرحلة، يُضاف لاحقاً.
  - **🟡 مؤجَّل — بادجات المنتجات (🏆 اختيار الشيف / 🔥 الأكثر طلباً):** مرتبطة بـ`is_chef_pick` وقسم الأكثر طلباً.
